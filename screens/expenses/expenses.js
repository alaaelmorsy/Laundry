window.addEventListener('DOMContentLoaded', () => {
  ...
  try {
    ...
    const DEFAULT_EXPENSE_CATEGORY = 'عام';
    let currentExpenseCategory = DEFAULT_EXPENSE_CATEGORY;
    ...
    inputDate.addEventListener('click', () => {
      inputDate.showPicker ? inputDate.showPicker() : inputDate.focus();
    });
    ...
    function formatExpenseDate(dateStr) {
      ...
      try {
        ...
        if (m) {
          ...
          return `${m[3]}/${m[2]}/${m[1]} ${hour12}:${minute} ${period}`;
        }
    ...
    function renderTable(expenses) {
      ...
      expensesTableBody.innerHTML = expenses.map((e, i) => `
        <tr>
          ...
          <td>${formatExpenseDate(e.expense_date)}</td>
    ...
    function openModal(expense) {
      ...
      inputDate.value = expense ? formatDateForInput(expense.expense_date) : getCurrentDateTime();
      inputNotes.value = expense ? (expense.notes || '') : '';
      inputIsTaxable.checked = expense ? Boolean(expense.is_taxable) : false;
      currentExpenseCategory = expense ? (expense.category || DEFAULT_EXPENSE_CATEGORY) : DEFAULT_EXPENSE_CATEGORY;
    ...
    function formatDateForInput(dateStr) {
      ...
      try {
        ...
        if (isNaN(date.getTime())) {
          ...
          return dm ? `${dm[1]}-${dm[2]}-${dm[3]}T00:00` : getCurrentDateTime();
        }
    ...
    async function saveExpense() {
      ...
      const category = currentExpenseCategory || DEFAULT_EXPENSE_CATEGORY;
      ...
      if (!title) {
        showModalError(I18N.t('expenses-err-title'));
        return;
      }
      if (isNaN(amount) || amount <= 0) {
        showModalError(I18N.t('expenses-err-amount'));
        return;
      }
      if (!expenseDate) {
        showModalError(I18N.t('expenses-err-date'));
        return;
      }
