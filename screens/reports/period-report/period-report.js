window.addEventListener('DOMContentLoaded', () => {
  ...
  const PAY_META = {
    ...
    subscription: { label: 'ايصالات الاستهلاك',               icon: '🔄', cls: 'pay-subscription' },
  ...
  function renderInvoicesPage(page) {
    ...
    if (!allInvoices.length) {
      tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;color:#94a3b8;padding:20px">${I18N.t('period-report-no-invoices')}</td></tr>`;
  ...
  function buildInvoicesTable(invoices) {
    allInvoices = invoices || [];
    invCurrentPage = 1;
    if (!allInvoices.length) return `<tr><td colspan="6" style="text-align:center;color:#94a3b8;padding:20px">${I18N.t('period-report-no-invoices')}</td></tr>`;
  ...
  function renderInvoiceModal(order, items, subscription) {
    ...
    const invVatAmountRow = document.getElementById('invVatAmountRow');
    const invTypeLabelRow = document.getElementById('invTypeLabelRow');
    const invCustVatRow = document.getElementById('invCustVatRow');
    const invNotesGridRow = document.getElementById('invNotesGridRow');
    const invNotesGridContent = document.getElementById('invNotesGridContent');
    ...
    if (invNotesGridRow) {
      if (order.notes) {
        if (invNotesGridContent) invNotesGridContent.textContent = order.notes;
    ...
    if (isInclusive && vatRate > 0) {
      ...
      if (discount > 0) {
        var existingRowsPR1 = invDiscRow ? invDiscRow.parentNode.querySelectorAll('.dynamic-disc-row') : [];
        for (var i = 0; i < existingRowsPR1.length; i++) existingRowsPR1[i].remove();
        
        if (discountLabel.startsWith('[')) {
          if (invDiscRow) invDiscRow.style.display = 'none';
          try {
            var arrPR1 = JSON.parse(discountLabel);
            arrPR1.forEach(function(dItem) {
      ...
      if (vatAmount > 0) {
        ...
        if (invVatAmountRow) invVatAmountRow.style.display = '';
        if (invTypeLabelRow) invTypeLabelRow.style.display = '';
        ...
        if (invVatAmountRow) invVatAmountRow.style.display = 'none';
        if (invTypeLabelRow) invTypeLabelRow.style.display = 'none';
      ...
      if (discount > 0) {
        var existingRowsPR2 = invDiscRow ? invDiscRow.parentNode.querySelectorAll('.dynamic-disc-row') : [];
        for (var i = 0; i < existingRowsPR2.length; i++) existingRowsPR2[i].remove();
        
        if (discountLabel.startsWith('[')) {
          if (invDiscRow) invDiscRow.style.display = 'none';
          try {
            var arrPR2 = JSON.parse(discountLabel);
            arrPR2.forEach(function(dItem) {
              var div = document.createElement('div');
              div.className = 'inv-total-row dynamic-disc-row';
              div.innerHTML = '<span class="inv-total-label">' + dItem.label + '</span><span class="inv-total-val" dir="ltr">' + sarFmtA(dItem.amount) + '</span>';
              invDiscRow.parentNode.insertBefore(div, invDiscRow);
  ...
  window.showConsumptionReceiptModal = async function(id) {
    ...
    try {
      ...
      if (g('crAmountLabel'))    g('crAmountLabel').textContent    = 'المبلغ المستهلك';
      if (g('crBalBeforeLabel')) g('crBalBeforeLabel').textContent = 'الرصيد قبل';
      if (g('crBalAfterLabel'))  g('crBalAfterLabel').textContent  = 'الرصيد بعد';
  ...
  function buildSubscriptionsTable(subscriptions) {
    if (!subscriptions.length) return `<tr><td colspan="6" style="text-align:center;color:#94a3b8;padding:20px">${I18N.t('period-report-no-subscriptions')}</td></tr>`;
  ...
  async function loadReport() {
    ...
    try {
      const res = await window.api.getReportData(currentFilters);
      if (!res.success) { showToast(res.message || I18N.t('all-invoices-err-load'), 'error'); loadingState.style.display = 'none'; return; }
      reportData = res;

      periodInfoBar.textContent = `${I18N.t('period-report-period')}: ${fmtD(df)} — ${fmtD(dt)}`;
      ...
      const consumptionInvoices = res.consumptionReceipts || [];
      const consTotal = consumptionInvoices.reduce((s, i) => s + Number(i.total_amount || 0), 0);
      const payMethodsFixed = (res.paymentMethods || []).map(m =>
        m.method === 'subscription'
          ? { ...m, count: consumptionInvoices.length, totalAfterTax: consTotal, totalBeforeTax: consTotal, totalTax: 0 }
          : m
      );
      if (!payMethodsFixed.some(m => m.method === 'subscription') && consumptionInvoices.length > 0) {
        payMethodsFixed.push({ method: 'subscription', count: consumptionInvoices.length, totalAfterTax: consTotal, totalBeforeTax: consTotal, totalTax: 0 });
      }
      ...
      document.getElementById('badgeConsumption').textContent = consumptionInvoices.length;
      document.getElementById('consumptionTableBody').innerHTML = buildConsumptionTable(consumptionInvoices);
      document.getElementById('consumptionFooter').innerHTML = `العدد: ${consumptionInvoices.length} &nbsp;|&nbsp; ${SAR(consTotal)}`;
  ...
  function rebuildReportContent() {
    if (!reportData) return;
    if (currentFilters) {
      periodInfoBar.textContent = `${I18N.t('period-report-period')}: ${fmtD(currentFilters.dateFrom)} — ${fmtD(currentFilters.dateTo)}`;
  ...
  btnExcelExport.addEventListener('click', async () => {
    ...
    const r = await window.api.exportReport({ type: 'excel', filters: currentFilters });
  ...
  btnPdfExport.addEventListener('click', async () => {
    ...
    const r = await window.api.exportReport({ type: 'pdf', filters: currentFilters });
